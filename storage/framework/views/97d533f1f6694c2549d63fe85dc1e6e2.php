...
<?php $__env->startSection('title', 'Manajemen Add-ons - RisingCare'); ?>

<?php $__env->startSection('content'); ?>
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Manajemen Add-ons Layanan</h1>
        <p class="text-gray-600">Kelola layanan tambahan yang dapat ditambahkan saat treatment</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <table class="w-full text-left">
        <thead class="bg-gray-50 border-b border-gray-200">
            <tr>
                <th class="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Nama Add-on</th>
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-4 font-medium text-gray-900"><?php echo e($addon->name); ?></td>
                <td class="px-6 py-4 text-gray-600"><?php echo e($addon->description ?? '-'); ?></td>
                <td class="px-6 py-4 font-medium text-gray-900">Rp <?php echo e(number_format($addon->price, 0, ',', '.')); ?></td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-sm font-medium">
                    <div class="flex space-x-3">
                        <button onclick="editAddon(<?php echo e($addon->id); ?>, '<?php echo e(addslashes($addon->name)); ?>', '<?php echo e(addslashes($addon->description)); ?>', <?php echo e($addon->price); ?>, <?php echo e($addon->is_active ? 'true' : 'false'); ?>)" class="text-blue-600 hover:text-blue-900">
                            <i class="fas fa-edit"></i>
                        </button>
                        <form id="deleteForm<?php echo e($addon->id); ?>" action="<?php echo e(route('admin.service-addons.destroy', $addon)); ?>" method="POST" class="inline">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="button" onclick="openDeleteModal('deleteForm<?php echo e($addon->id); ?>', '<?php echo e(addslashes($addon->name)); ?>')" class="text-red-600 hover:text-red-900"><i class="fas fa-trash"></i></button>
                        </form>
                    </div>
                </td>
            </tr>
...
<div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Tambah Add-on Baru</h3>
            <form action="<?php echo e(route('admin.service-addons.store')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Nama Add-on</label>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="price" min="0" step="1000" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Edit Add-on</h3>
            <form id="editForm" method="POST">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Harga</label>
                    <input type="number" name="price" id="editPrice" min="0" step="1000" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="mb-4">
...
<script>
    ...
    function editAddon(id, name, description, price, isActive) {
        document.getElementById('editName').value = name;
        document.getElementById('editDescription').value = description;
        document.getElementById('editPrice').value = price;
        document.getElementById('editActive').checked = isActive;
        document.getElementById('editForm').action = `/admin/service-addons/${id}`;
        openModal('editModal');
    }
</script>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Project\laragon\www\risingcare\resources\views/admin/service-addons/index.blade.php ENDPATH**/ ?>