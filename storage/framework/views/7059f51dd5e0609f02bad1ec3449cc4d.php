...
<?php $__env->startSection('title', 'Services Highlight - RisingCare'); ?>

<?php $__env->startSection('content'); ?>
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Services Highlight</h1>
        <p class="text-gray-600">Kelola highlight layanan di landing page</p>
    </div>
    <button onclick="openCreateModal()" class="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition">
        <i class="fas fa-plus mr-2"></i> Tambah Service Highlight
    </button>
</div>
    ...
    <form action="<?php echo e(route('admin.settings.update')); ?>" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="settings[landing_services_title]" value="<?php echo e($sectionTitle ?? 'Layanan Kami'); ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                <label class="block text-gray-700 font-medium mb-2">Section Subtitle</label>
                <input type="text" name="settings[landing_services_subtitle]" value="<?php echo e($sectionSubtitle ?? 'Layanan kesehatan profesional dan terpercaya'); ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php $__empty_1 = true; $__currentLoopData = $servicesHighlight; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900 font-medium"><?php echo e($item->order); ?></td>
                    <td class="px-6 py-4">
                        <?php if($item->icon): ?>
                            <i class="fas fa-<?php echo e($item->icon); ?> text-2xl text-teal-600"></i>
                        <?php else: ?>
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs font-medium <?php echo e($item->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?> rounded-full">
                    ...
                    <td class="px-6 py-4 text-sm font-medium">
                        <div class="flex space-x-3">
                            <button type="button" onclick="openEditModal(<?php echo e($item->id); ?>)" class="text-blue-600 hover:text-blue-900">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form id="deleteForm<?php echo e($item->id); ?>" action="<?php echo e(route('admin.landing.services-highlight.destroy', $item)); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="button" onclick="if(confirm('Yakin ingin menghapus?')) document.getElementById('deleteForm<?php echo e($item->id); ?>').submit()" class="text-red-600 hover:text-red-900">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                        Belum ada service highlight yang ditambahkan
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white">
            <h2 class="text-xl font-bold text-gray-800">Tambah Service Highlight</h2>
            <button type="button" onclick="closeCreateModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        
        <form id="createForm" method="POST" action="<?php echo e(route('admin.landing.services-highlight.store')); ?>">
            <?php echo csrf_field(); ?>
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="icon" placeholder="e.g., stethoscope" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                ...
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Link ke Service</label>
                    <select name="service_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white">
            <h2 class="text-xl font-bold text-gray-800">Edit Service Highlight</h2>
        ...
        <form id="editForm" method="POST" action="">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Icon (Font Awesome)</label>
                    <input type="text" name="icon" id="edit_icon" placeholder="e.g., stethoscope" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                </div>
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Description <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-gray-700 font-medium mb-2">Link ke Service</label>
                    <select name="service_id" id="edit_service_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                        <option value="">- Tidak ada -</option>
                        <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($service->id); ?>"><?php echo e($service->name); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
function openEditModal(id) {
    fetch(`/admin/landing/services-highlight/${id}/edit`, {
        headers: {
    ...
    .then(data => {
        ...
        document.getElementById('editForm').action = `/admin/landing/services-highlight/${id}`;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Project\laragon\www\risingcare\resources\views/admin/landing/services-highlight/index.blade.php ENDPATH**/ ?>