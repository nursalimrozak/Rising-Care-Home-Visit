...
@section('title', 'Detail Customer - RisingCare')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="text-gray-900">
                        {{ $user->date_of_birth ? \Carbon\Carbon::parse($user->date_of_birth)->format('d F Y') : '-' }}
                        @if($user->date_of_birth)
                            <span class="text-gray-500 text-sm">({{ \Carbon\Carbon::parse($user->date_of_birth)->age }} tahun)</span>
                        @endif
                    </p>
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">Nomor BPJS</label>
                    <p class="text-gray-900">{{ $user->bpjs_number ?? '-' }}</p>
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">Jenis Kelamin</label>
                    <p class="text-gray-900">{{ $user->gender == 'male' ? 'Laki-laki' : ($user->gender == 'female' ? 'Perempuan' : '-') }}</p>
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">Pekerjaan</label>
                    <p class="text-gray-900">{{ $user->occupation->name ?? '-' }}</p>
                </div>
                <div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="text-gray-900">{{ $user->emergency_contact_relationship ?? '-' }}</p>
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">No. Telepon</label>
                ...
                <div class="md:col-span-2">
                    <label class="text-sm text-gray-500 block mb-1">Alamat</label>
                    <p class="text-gray-900">{{ $user->emergency_contact_address ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            @if($user->healthRecords->count() > 0)
                <div class="space-y-4">
                    @foreach($user->healthRecords as $record)
                    <div class="border-l-4 border-teal-500 bg-gray-50 p-4 rounded">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <h3 class="font-semibold text-gray-900">{{ $record->category->name }}</h3>
                                <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($record->recorded_at)->format('d M Y H:i') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-3">
                            ...
                            <div>
                                <p class="text-xs text-gray-500">{{ $answer['question'] }}</p>
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200">
                            <p class="text-sm text-gray-600"><strong>Catatan:</strong> {{ $record->notes }}</p>
                        </div>
                        @endif
                    </div>
                    @endforeach
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-gray-900">{{ $user->bookingsAsCustomer->count() }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Booking Selesai</span>
                    <span class="font-bold text-green-600">{{ $user->bookingsAsCustomer->where('status', 'completed')->count() }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Rekap Kesehatan</span>
                    <span class="font-bold text-teal-600">{{ $user->healthRecords->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl shadow-sm p-6 text-white">
            ...
            <p class="text-2xl font-bold">{{ $user->membership->name }}</p>
            <p class="text-sm opacity-90 mt-1">Diskon {{ $user->membership->discount_percentage }}%</p>
        </div>
        @endif
    </div>
</div>
