            ...
            <div class="px-6 pb-6">
                ...
                <div class="text-center mb-6">
                    ...
                    <div class="mt-3">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $user->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            <span class="w-2 h-2 mr-1.5 rounded-full {{ $user->is_active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                ...
                <div class="space-y-4 border-t border-gray-100 pt-6">
                    <div class="flex items-center text-sm">
                        ...
                        <span class="text-gray-600 truncate">{{ $user->phone ?? '-' }}</span>
                    </div>
                    <div class="flex items-center text-sm">
                        <div class="w-8 flex-shrink-0 text-center text-gray-400">
                    ...
                    <div class="flex items-center text-sm">
                        ...
                        <span class="text-gray-600">{{ $user->date_of_birth ? \Carbon\Carbon::parse($user->date_of_birth)->format('d M Y') : '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                @foreach($docTypes as $type => $label)
                    <div class="flex items-center justify-between p-3 rounded-xl {{ isset($documents[$type]) ? 'bg-green-50 border border-green-100' : 'bg-gray-50 border border-gray-100' }}">
                        <div class="flex items-center">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center {{ isset($documents[$type]) ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-400' }}">
                                <i class="fas {{ isset($documents[$type]) ? 'fa-check' : 'fa-times' }} text-xs"></i>
                            </div>
                            <span class="ml-3 text-sm font-medium {{ isset($documents[$type]) ? 'text-gray-900' : 'text-gray-500' }}">{{ $label }}</span>
                        </div>
                        @if(isset($documents[$type]))
                            <a href="{{ asset('storage/' . $documents[$type]->file_path) }}" target="_blank" class="text-teal-600 hover:text-teal-800 text-xs font-medium">Lihat</a>
                        @endif
                    </div>
                @endforeach
            </div>
    ...
    <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 text-white shadow-lg relative overflow-hidden group">
                            ...
                            <div class="relative z-10">
                                ...
                                <p class="text-xl font-bold mb-6">{{ $user->payoutDetail->provider_name }}</p>
                                
                                <p class="text-gray-400 text-xs uppercase tracking-wider mb-1">Nomor Akun</p>
                                <p class="text-lg font-mono tracking-wider mb-4">{{ $user->payoutDetail->account_number }}</p>
                                
                                <div class="flex justify-between items-end">
                                    <div>
                                        <p class="text-gray-400 text-xs uppercase tracking-wider mb-1">Atas Nama</p>
                                        <p class="font-medium">{{ $user->payoutDetail->account_holder_name }}</p>
                                    </div>
                                    <div class="bg-white/20 px-2 py-1 rounded text-xs font-bold uppercase">
                                        {{ $user->payoutDetail->payment_type }}
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <p class="text-2xl font-bold text-gray-900">Rp {{ number_format($user->commissions->sum('amount'), 0, ',', '.') }}</p>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <p class="text-sm text-gray-500 mb-1">Booking Selesai</p>
                <p class="text-2xl font-bold text-gray-900">{{ $user->bookingsAsPetugas()->where('status', 'completed')->count() }}</p>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="px-6 py-6">
                        ...
                        <div x-show="tab === 'personal'" class="space-y-5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div>
                                    ...
                                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('name') border-red-500 @enderror">
                                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('email') border-red-500 @enderror">
                                    @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">No. Telepon</label>
                                    <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('phone') border-red-500 @enderror">
                                    @error('phone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Lahir</label>
                                    <input type="date" name="date_of_birth" value="{{ old('date_of_birth', $user->date_of_birth) }}" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('date_of_birth') border-red-500 @enderror">
                                    @error('date_of_birth') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Kelamin</label>
                                    <select name="gender" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('gender') border-red-500 @enderror">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="address" rows="2" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('address') border-red-500 @enderror">{{ old('address', $user->address) }}</textarea>
                        ...
                        <div x-show="tab === 'payment'" class="space-y-5">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="provider_name" value="{{ old('provider_name', $user->payoutDetail->provider_name ?? '') }}" placeholder="Contoh: BCA, Mandiri, OVO, Dana" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('provider_name') border-red-500 @enderror">
                                    @error('provider_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="account_number" value="{{ old('account_number', $user->payoutDetail->account_number ?? '') }}" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('account_number') border-red-500 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="account_holder_name" value="{{ old('account_holder_name', $user->payoutDetail->account_holder_name ?? '') }}" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('account_holder_name') border-red-500 @enderror">
                        ...
                        <div x-show="tab === 'password'" class="space-y-5">
                            <div>
                                ...
                                <input type="password" name="current_password" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('current_password') border-red-500 @enderror">
                                @error('current_password') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Password Baru</label>
                                <input type="password" name="new_password" class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all @error('new_password') border-red-500 @enderror">
