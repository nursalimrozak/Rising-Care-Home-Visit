...
@section('title', 'Detail User - RisingCare')
        ...
        <div class="px-6 pb-6 relative">
            <div class="-mt-12 mb-4 flex justify-between items-end">
                <div class="h-24 w-24 rounded-full bg-white p-1 shadow-lg">
                    @if($user->avatar)
                        <img src="{{ asset('storage/' . $user->avatar) }}" alt="{{ $user->name }}" class="h-full w-full rounded-full object-cover">
                    @else
            ...
            <p class="text-gray-500 mb-4">{{ optional($user->occupation)->name ?? 'User' }}</p>
            ...
            <div class="space-y-3 border-t pt-4">
                ...
                <div class="flex items-center text-gray-600">
                    ...
                    <a href="https://wa.me/{{ preg_replace('/^0/', '62', preg_replace('/[^0-9]/', '', $user->phone)) }}" target="_blank" class="hover:text-teal-600 hover:underline">
                        {{ $user->phone }} <i class="fas fa-external-link-alt text-xs ml-1"></i>
                    </a>
                </div>
                @if($user->address)
            ...
            @if($user->role != 'customer' && $user->payoutDetail)
            <div class="mt-6 bg-blue-50 rounded-lg p-4 border border-blue-100">
                ...
                <div class="space-y-2">
                    @if($user->payoutDetail->provider_name)
                    <div class="flex justify-between text-sm">
                        <span class="text-blue-700">Bank/E-Wallet:</span>
                        <span class="font-medium text-blue-900">{{ $user->payoutDetail->provider_name }}</span>
                    </div>
                    @endif
                    @if($user->payoutDetail->account_number)
                    <div class="flex justify-between text-sm">
                        <span class="text-blue-700">No. Rekening:</span>
                        <span class="font-medium text-blue-900 font-mono">{{ $user->payoutDetail->account_number }}</span>
                    </div>
                    @endif
                    @if($user->payoutDetail->account_holder_name)
                    <div class="flex justify-between text-sm">
                        <span class="text-blue-700">Atas Nama:</span>
                        <span class="font-medium text-blue-900">{{ $user->payoutDetail->account_holder_name }}</span>
                    </div>
                    @endif
                </div>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-2 bg-white rounded border border-gray-200">
                        <div class="flex items-center overflow-hidden">
                            ...
                            <div class="truncate">
                                <p class="text-sm font-medium text-gray-700 truncate" title="{{ $doc->original_name ?? $doc->document_type }}">
                                    {{ $doc->document_type }}
                                </p>
                                <p class="text-xs text-gray-500 truncate">{{ $doc->original_name }}</p>
                            </div>
                        </div>
                        <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" class="text-teal-600 hover:text-teal-800 ml-2 flex-shrink-0" title="Lihat Dokumen">
            ...
            <div class="mt-6 bg-yellow-50 rounded-lg p-4 border border-yellow-100">
                ...
                <p class="text-3xl font-bold text-yellow-600">{{ $user->loyalty_points }}</p>
                <p class="text-xs text-yellow-700 mt-1">Dapat ditukar dengan voucher diskon</p>
            </div>
            @endif

            <div class="mt-6 flex gap-2">
                <a href="{{ route('admin.users.edit', $user) }}" class="flex-1 bg-blue-600 text-white py-2 rounded-lg text-center hover:bg-blue-700 transition">
                    Edit Profil
                </a>
            </div>
        </div>
    ...
    <div class="lg:col-span-2 space-y-6">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-800">
                                {{ optional($booking->service)->name ?? 'Service Deleted' }}
                            </td>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                @forelse($user->loyaltyTransactions as $transaction)
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                ...
                <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    {{ $user->bookingsAsPetugas->count() }} Pasien
                </span>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($user->bookingsAsPetugas as $booking)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ optional($booking->customer)->name ?? 'Unknown Customer' }}</div>
                                <div class="text-xs text-gray-500">{{ $booking->booking_number }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-800">
                                <div class="font-medium">{{ optional($booking->service)->name ?? 'Service Deleted' }}</div>
                                @if($booking->packagePurchase)
                                    <div class="text-xs text-teal-600 mt-1">
                                        <i class="fas fa-box mr-1"></i>
                                        Paket: {{ $booking->packagePurchase->package_name }} 
                                        ({{ $booking->packagePurchase->total_sessions }}x)
                                        - Kunjungan ke-{{ $booking->visit_number }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.bookings.show', $booking) }}" class="text-teal-600 hover:text-teal-900 text-sm font-medium">Detail</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
