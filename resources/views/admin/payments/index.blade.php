...
@section('title', 'Daftar Pembayaran - RisingCare')
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="text-xs text-gray-500">{{ $payment->booking->booking_number ?? '-' }}</div>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-600">
                        @if($payment->booking->packagePurchase)
                            <span class="font-medium text-teal-600">{{ $payment->booking->packagePurchase->package->name ?? 'Paket' }}</span>
                            <div class="text-xs text-gray-500">{{ $payment->booking->service->name ?? '-' }}</div>
                        @else
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ route('admin.payments.show', $payment) }}" class="text-blue-600 hover:text-blue-900 font-medium text-sm">
                            Detail
                        </a>
                    </td>
                </tr>
