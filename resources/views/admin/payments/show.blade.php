...
@section('title', 'Detail Pembayaran - RisingCare')

@section('content')
<div class="mb-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">Detail Pembayaran #{{ $payment->payment_number }}</h1>
        <a href="{{ route('admin.payments.index') }}" class="text-teal-600 hover:text-teal-800 font-medium flex items-center">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600">{{ $payment->booking->customer->email ?? '-' }}</p>
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">Booking</label>
                    <p class="font-medium text-gray-900">{{ $payment->booking->booking_number ?? '-' }}</p>
                    <p class="text-sm text-gray-600">{{ $payment->booking->service->name ?? '-' }}</p>
                </div>
                <div>
                    ...
                    @elseif($payment->payment_type == 'addon')
                        <span class="px-3 py-1 rounded-full text-xs font-medium bg-teal-100 text-teal-800">Pembayaran Add-ons</span>
                    @else
                        <span class="px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{{ ucfirst($payment->payment_type) }}</span>
                    @endif
                </div>
                <div>
                    <label class="text-sm text-gray-500 block mb-1">Jumlah</label>
                    <p class="font-medium text-gray-900 text-lg">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    @else
                        <span class="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{{ ucfirst($payment->status) }}</span>
                    @endif
                </div>
                <div>
        ...
        {{-- Add-ons Details (if payment type is addon) --}}
        @if($payment->payment_type == 'addon' && $payment->booking->bookingAddons->count() > 0)
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-800 mb-4">Detail Add-ons yang Dibayar</h2>
            <div class="space-y-2">
                @foreach($payment->booking->bookingAddons as $bookingAddon)
                <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <div>
                        <p class="font-medium text-gray-900">{{ $bookingAddon->addon->name }}</p>
                        <p class="text-sm text-gray-500">{{ $bookingAddon->quantity }} x Rp {{ number_format($bookingAddon->price_per_item, 0, ',', '.') }}</p>
                    </div>
                    <p class="font-semibold text-teal-600">Rp {{ number_format($bookingAddon->subtotal, 0, ',', '.') }}</p>
                </div>
                @endforeach
                <div class="flex justify-between items-center pt-3 border-t border-gray-200">
                    <p class="font-bold text-gray-900">Total Add-ons:</p>
                    <p class="font-bold text-teal-600 text-lg">Rp {{ number_format($payment->booking->bookingAddons->sum('subtotal'), 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            @if($payment->payment_proof)
                <div class="flex flex-col items-center">
                    <img src="{{ asset('storage/' . $payment->payment_proof) }}" alt="Bukti Pembayaran" class="max-w-xs w-full rounded-lg border border-gray-200 mb-3 cursor-pointer hover:opacity-90 transition" onclick="openImageModal('{{ asset('storage/' . $payment->payment_proof) }}')">
                    <button onclick="openImageModal('{{ asset('storage/' . $payment->payment_proof) }}')" class="text-sm text-blue-600 hover:underline">
