...
@section('title', 'Articles - RisingCare')
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="settings[landing_articles_title]" value="{{ $sectionTitle ?? 'Artikel & Berita' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                <label class="block text-gray-700 font-medium mb-2">Section Subtitle</label>
                <input type="text" name="settings[landing_articles_subtitle]" value="{{ $sectionSubtitle ?? 'Informasi kesehatan terkini untuk Anda dan keluarga' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
        </div>
        ...
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            @if($article->featured_image)
                                <img src="{{ asset('storage/' . $article->featured_image) }}" alt="{{ $article->title }}" class="w-12 h-12 rounded object-cover mr-3">
                            @else
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $article->is_published ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                            {{ $article->is_published ? 'Published' : 'Draft' }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <button onclick="openEditModal({{ $article->id }})" class="text-blue-600 hover:text-blue-800">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button type="button" onclick="openGlobalDeleteModal('deleteForm{{ $article->id }}', '{{ addslashes($article->title) }}', 'Hapus Artikel')" class="text-red-600 hover:text-red-800">
                                <i class="fas fa-trash"></i>
                            </button>
                            <form id="deleteForm{{ $article->id }}" action="{{ route('admin.landing.articles.destroy', $article->id) }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form id="createForm" method="POST" action="{{ route('admin.landing.articles.store') }}" enctype="multipart/form-data">
            @csrf
