...
@section('title', 'Services Highlight - RisingCare')
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="settings[landing_services_title]" value="{{ $sectionTitle ?? 'Layanan Kami' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            <div>
                <label class="block text-gray-700 font-medium mb-2">Section Subtitle</label>
                <input type="text" name="settings[landing_services_subtitle]" value="{{ $sectionSubtitle ?? 'Layanan kesehatan profesional dan terpercaya' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($servicesHighlight as $item)
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm text-gray-900 font-medium">{{ $item->order }}</td>
                    <td class="px-6 py-4">
                        @if($item->icon)
                            <i class="fas fa-{{ $item->icon }} text-2xl text-teal-600"></i>
                        @else
                    ...
                    <td class="px-6 py-4 text-sm font-medium">
                        <div class="flex space-x-3">
                            ...
                            <form id="deleteForm{{ $item->id }}" action="{{ route('admin.landing.services-highlight.destroy', $item) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="if(confirm('Yakin ingin menghapus?')) document.getElementById('deleteForm{{ $item->id }}').submit()" class="text-red-600 hover:text-red-900">
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form id="createForm" method="POST" action="{{ route('admin.landing.services-highlight.store') }}">
            @csrf
