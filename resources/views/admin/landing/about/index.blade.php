...
@section('title', 'About Section - RisingCare')
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <form action="{{ route('admin.landing.about.update') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="settings[landing_about_title]" value="{{ $settings['landing_about_title'] ?? 'Empower Your Life With Healthcare Care' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Description</label>
                    <textarea name="settings[landing_about_description]" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">{{ $settings['landing_about_description'] ?? 'RisingCare menyediakan layanan kesehatan berkualitas dengan tenaga medis profesional dan berpengalaman. Kami berkomitmen untuk memberikan perawatan terbaik untuk Anda dan keluarga.' }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div id="points-container" class="space-y-3">
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="points[]" value="{{ $point }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            ...
            <div>
                ...
                <div class="bg-gray-50 rounded-xl p-4 border-2 border-dashed border-gray-300 text-center">
                    @if(isset($settings['landing_about_image']))
                        <img src="{{ asset('storage/' . $settings['landing_about_image']) }}" alt="About Image" class="max-h-64 mx-auto rounded-lg mb-4">
                    @else
